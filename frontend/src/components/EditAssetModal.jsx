...
const assetCategories = {
  ASSET: ['Bank', 'Cash', 'Property', 'Vehicle', 'Jewelry', 'Electronics', 'Others'],
  LIABILITY: ['Credit Card', 'Personal Loan', 'Car Loan', 'Home Loan', 'Others'],
  DEBT: ['Education Loan', 'Business Loan', 'Medical Debt', 'Others'],
  INVESTMENT: ['Stocks', 'Mutual Funds', 'Fixed Deposit', 'Real Estate', 'Gold', 'Cryptocurrency', 'Others'],
};
...
export default function EditAssetModal({ asset, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    await onSave(asset.id, { ...form, value: parseFloat(form.value) });
    setLoading(false);
    onClose();
  };

  const categories = assetCategories[form.type] || ['Others'];
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl p-8 shadow-2xl relative">
        ...
        <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-6 text-center">Edit Asset/Liability</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase block mb-1.5">Type</label>
            <select
              value={form.type}
              onChange={(e) => setForm({ ...form, type: e.target.value, category: assetCategories[e.target.value]?.[0] || 'Others' })}
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase block mb-1.5">Description (Optional)</label>
            <textarea
              placeholder="Additional notes"
              value={form.description}
              onChange={(e) => setForm({ ...form, description: e.target.value })}
              className="w-full border border-gray-200 dark:border-gray-600 rounded-xl p-3 bg-gray-50 dark:bg-gray-700 outline-none focus:ring-2 focus:ring-slate-300 dark:focus:ring-slate-600 text-sm dark:text-white resize-none"
              rows="3"
            />
          </div>
