  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white w-full max-w-md rounded-2xl p-8 shadow-2xl relative">
        ...
        <h2 className="text-lg font-bold text-slate-800 mb-6 text-center">Add {assetType.charAt(0) + assetType.slice(1).toLowerCase()}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">Name</label>
            <input
              required
              type="text"
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">Value (₹)</label>
            <input
              required
              type="number"
              min="1"
